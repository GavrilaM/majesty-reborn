...
export class Stats {
    ...
    update() {
        ...
        // STAMINA: Endurance system (Physical classes benefit more)
        // REBALANCED: Slower regen to make stamina management matter
        const classBonus = (this.classType === 'MERCENARY' || this.classType === 'WARRIOR') ? 30
            : ((this.classType === 'WAYFARER' || this.classType === 'RANGER') ? 20 : 10);
        this.derived.staminaMax = 100 + (this.current.VIT * 15) + (this.current.AGI * 5) + classBonus;
        // Reduced base from 5 to 2, and coefficients halved
        this.derived.staminaRegen = 2 + (this.current.VIT * 0.2) + (this.current.WIL * 0.1);
